...
ON_EVENT(CWebBrowserUIDlg, IDC_WEBBROWSER, 259, CWebBrowserUIDlg::OnDocumentcompleteWebbrowser, VTS_DISPATCH VTS_PVARIANT)
END_EVENTSINK_MAP()
...
void CWebBrowserUIDlg::OnDocumentcompleteWebbrowser(LPDISPATCH pDisp, VARIANT* URL)
{
	HRESULT   hr;
	LPUNKNOWN lpUnknown;
	...
	if (lpUnknown)
	{
		...
		if (SUCCEEDED(hr) && lpUnknownWB == lpUnknownDC)
		{
			...
			IHTMLDocument2* pDoc = (IHTMLDocument2*)m_WebBrowser.get_Document();
